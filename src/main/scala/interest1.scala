...
case class LikeMusic(uid : String, interest : String)

object Interest1{

  val musicteam = "SHE,五月天,花儿乐队,飞儿乐团,信乐团,5566,羽泉,水木年华,BOBO,小虎队,棒棒堂,南拳妈妈,中国娃娃,苏打绿,动力火车,F4,Twins,大嘴巴,芭比,黑涩会美眉,麻吉弟弟,Beyond,小f4,女子十二乐坊,Boys,零点乐队,闪亮三姐妹,黑豹"
  val musicstar="周杰伦,林俊杰,歌星,张学友,陈奕迅,胡月,那英,毛阿敏,许嵩,徐良,汪苏泷"
  val musicterm="流行歌曲,我们的爱,大海,海阔天空,飘雪,你的样子,上海滩,偏偏喜欢你,吻别,水手"
  val muterm = (musicteam + "," + musicstar + "," + musicterm).split(",")

  var mMap = Map[String, Int]()
  for ( i <- 1 to muterm.length ){
    mMap.+(muterm(i-1)->1)
  }
  def containsMU(input : String)={
    var tag = false
    for ( i <- 1 to muterm.length )
      if ( input.contains(muterm(i-1)) )
        tag = true
    tag
  }

  def containsMUS(input : String)={
    muterm.map(input.contains(_)).reduce(_||_)
  }
  ...
  def main(argv:Array[String]) : Unit = {
    ...
    val weibo = sc.textFile("weibo.txt").map(_.split("\t")).filter(_.length>5).filter(a=>containsMUS(a(5))).map(a=>(a(1),1)).reduceByKey(_+_).filter(_._2>5).map(a=> LikeMusic(a._1,"music")).toDF
    val resDF = up.join(weibo,"uid")
    val prop = new java.util.Properties
    prop.setProperty("serverTimezone", "UTC")
    prop.setProperty("user","root")
    prop.setProperty("password","971103")
    resDF.write.mode(SaveMode.Overwrite).jdbc("jdbc:mysql://127.0.0.1:3306/test?useUnicode=true&characterEncoding=utf-8", "music", prop)
